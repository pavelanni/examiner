...
func (h *Handler) handleTeacherUpload(w http.ResponseWriter, r *http.Request) {
    ...
    absSchema, _ := filepath.Abs("schema/question_schema.json")
    compiler := jsonschema.NewCompiler()
    f, err := os.Open(absSchema)
    if err != nil {
        slog.Warn("failed to open schema, falling back to basic JSON unmarshal", "error", err)
    } else {
        defer f.Close()
        schemaURL := "file://" + filepath.ToSlash(absSchema)
        if err := compiler.AddResource(schemaURL, f); err != nil {
            slog.Warn("failed to add schema resource", "error", err)
        } else {
            sch, err := compiler.Compile(schemaURL)
            if err != nil {
                slog.Warn("failed to compile schema", "error", err)
            } else {
                var v interface{}
    ...
    // Выполняем автоматический редирект на профиль преподавателя
